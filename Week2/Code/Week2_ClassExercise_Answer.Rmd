---
title: "Week 2: Class Exercise on Stationarity & Seasonality"
...
head(DJIA)
tail(DJIA)
class(DJIA) #notice it's a xts and zoo object
...
Can you examine the moments of this time series? Feel free to use plots & statistical tests.


Should we detrend the series?
```{r}
...
```
So, is this white noise?  Remember we are dealing with a null hypothesis of no serial correlation.  If we get a pvalue greater than our threshhold (say 5% for 95% confidence), then we fail to reject the null.  
